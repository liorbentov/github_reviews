...
export const getPRsData = async (startDate: string) => {
    const start = Moment(startDate);
    const data: PR[] = await getPRs();
    return data
...
export const setPRs = async (startDate: string, setReviewers: ({}) => void) => {
    const prsNumbers = await getPRsData(startDate.toString());
    const data = await Promise.all(
        prsNumbers.map(async prNumber => {
            const reviews: Review[] = await getReviews(prNumber);
